...
Glide defaults to `gpt-5.6-luna` and `https://api.openai.com/v1/responses`. Terra and Sol can be selected through `glide.model`. A controlled Responses-compatible endpoint can be supplied through the advanced `glide.endpoint` setting. Glide requires HTTPS except for loopback development endpoints.
...
```json
{
  "glide.endpoint": "https://example/v1/responses",
